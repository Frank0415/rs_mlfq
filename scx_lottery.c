...
const char help_fmt[] = "彩票调度器，根据任务的优先级分配彩票数量\n"  // 帮助信息格式字符串
			"\n"  // 空行
			"使用方法: %s [-v] [-h]\n"  // 使用方法说明
			"\n"  // 空行
			"  -v            打印 libbpf 调试信息\n"  // -v选项说明
...
{
	if (level == LIBBPF_DEBUG && !verbose)  // 如果是DEBUG级别且不启用详细模式
		return 0;  // 返回0不打印
...
static void read_stats(struct scx_lottery_bpf *skel, __u64 *stats)  // 读取统计信息的函数
{
	...
	for (idx = 0; idx < 2; idx++) {  // 遍历2个统计项
		...
		ret = bpf_map_lookup_elem(bpf_map__fd(skel->maps.stats), &idx,  // 从BPF映射中查找统计值
					  cnts[idx]);  // 存储到计数数组
		if (ret < 0)  // 如果查找失败
			continue;  // 继续下一个循环
...
restart:  // 重启标签（用于调度器重启）
	...
	while ((opt = getopt(argc, argv, "vh")) != -1) {  // 解析命令行参数
		...
		case 'h':  // -h选项：显示帮助
		default:  // 默认情况或-h选项
			fprintf(stderr, help_fmt, basename(argv[0]));  // 打印帮助信息
			return opt != 'h';  // 如果是-h则返回0成功，否则返回1失败
		}  // switch结束
	}  // while循环结束

	SCX_OPS_LOAD(skel, lottery_ops, scx_lottery_bpf, uei);  // 加载BPF程序
	link = SCX_OPS_ATTACH(skel, lottery_ops, scx_lottery_bpf);  // 附加BPF程序

	printf("彩票调度器已启动，按 Ctrl+C 退出\n");  // 打印启动信息

	while (!exit_req && !UEI_EXITED(skel, uei)) {  // 当不退出且BPF程序未退出时循环
